...
main:
	...
	# Throw Exception: Array size is <= 0
	  call_pal	0x555		# (exception: Array size is <= 0)
	  call_pal	0x555		# (halt)
__L0:
	# _tmp2 = _tmp0 + 1
	  ldq		$r1, -32($r15)	# fill _tmp0 to $r1 from $r15-32
	...
	# Throw Exception: Array subscript out of bounds
	  call_pal	0x555		# (exception: Array subscript out of bounds)
	  call_pal	0x555		# (halt)
__L1:
	# _tmp10 = _tmp5 << 3
	  ldq		$r1, -72($r15)	# fill _tmp5 to $r1 from $r15-72
...
__Alloc:
    ...
    __AllocReturnCheck:
	  ...
	  call_pal	0x555		# (exception: this really should not happen in Malloc)
	  call_pal	0x555		# (halt)
    __AllocReturnGood:
    ...
    __AllocReturnFail:
	  call_pal	0x555		# (exception: Malloc failed to find space in heap)
	  call_pal	0x555		# (halt)
	# EndFunc
