...
static int cycle_count_core1 = 0;
static FILE* ppfile = NULL;
static FILE* ppfile_core1 = NULL;
...
void print_header_core1(char* str)
{
  if (ppfile_core1 == NULL)
    ppfile_core1 = fopen("pipeline_core1.out", "w");

  fprintf(ppfile_core1, "%s", str);
}
...
void print_cycles_core1()
{
  /* we'll enforce the printing of a header */
  if (ppfile_core1 != NULL)
    fprintf(ppfile_core1, "\n%5d:", cycle_count_core1++);
}

void print_stage(char* div, int inst, int npc, int valid_inst, int core_num)
{
  int opcode, check;
  char *str;
  ...
  if (ppfile != NULL && core_num == 0)
    fprintf(ppfile, "%s%4d:%-8s", div, npc, str);
  if (ppfile != NULL && core_num == 1)
    fprintf(ppfile_core1, "%s%4d:%-8s", div, npc, str);  
}
...
void print_reg_core1(int wb_reg_wr_data_out_hi, int wb_reg_wr_data_out_lo,
                  int wb_reg_wr_idx_out, int wb_reg_wr_en_out)
{
  if (ppfile_core1 == NULL)
    return;
  ...
  if(wb_reg_wr_en_out)
    if((wb_reg_wr_data_out_hi==0)||
       ((wb_reg_wr_data_out_hi==-1)&&(wb_reg_wr_data_out_lo<0)))
      fprintf(ppfile_core1, "r%d=%d  ",wb_reg_wr_idx_out,wb_reg_wr_data_out_lo);
    else 
      fprintf(ppfile_core1, "r%d=0x%x%x  ",wb_reg_wr_idx_out,
              wb_reg_wr_data_out_hi,wb_reg_wr_data_out_lo);
...
void print_membus_core1(int proc2mem_command, int mem2proc_response,
                  int proc2mem_addr_hi, int proc2mem_addr_lo,
                  int proc2mem_data_hi, int proc2mem_data_lo)
{
  if (ppfile_core1 == NULL)
  ...
  switch(proc2mem_command)
  {
    case 1: fprintf(ppfile_core1, "BUS_LOAD  MEM["); break;
    case 2: fprintf(ppfile_core1, "BUS_STORE MEM["); break;
    default: return; break;
  }
  
  if((proc2mem_addr_hi==0)||
     ((proc2mem_addr_hi==-1)&&(proc2mem_addr_lo<0)))
    fprintf(ppfile_core1, "%d",proc2mem_addr_lo);
  else
    fprintf(ppfile_core1, "0x%x%x",proc2mem_addr_hi,proc2mem_addr_lo);
  if(proc2mem_command==1)
  {
    fprintf(ppfile_core1, "]");
  } else {
    ...
    if((proc2mem_data_hi==0)||
       ((proc2mem_data_hi==-1)&&(proc2mem_data_lo<0)))
      fprintf(ppfile_core1, "%d",proc2mem_data_lo);
    else
      fprintf(ppfile_core1, "0x%x%x",proc2mem_data_hi,proc2mem_data_lo);
  }
  if(mem2proc_response) {
    fprintf(ppfile_core1, " accepted %d",mem2proc_response);
  } else {
    fprintf(ppfile_core1, " rejected");
  }
}