...
int num_id_regs = 0;
int num_id_rs_regs = 0;
int num_rs_regs = 0;
int num_rs_ex_regs = 0;
int num_ex_regs = 0;
...
char **id_reg_names;
char **id_rs_reg_names;
char **rs_reg_names;
char **rs_ex_reg_names;
char **ex_reg_names;
...
void setup_gui(FILE *fp){
  ...
  arf_win = create_newwin(NUM_ARF+2,25,14-6,COLS-25,5);
  mvwprintw(arf_win,0,13,"PRF");
  int i=0;
  char tmp_buf[32];
  for (; i < NUM_ARF; i++) {
  ...
  mvwprintw(pipe_win,1,3*pipe_width-2,"RS");
  mvwprintw(pipe_win,1,4*pipe_width-3,"EX");
  mvwprintw(pipe_win,1,5*pipe_width-3,"MEM");
  ...
  //instantiate a window to visualize ID/RS signals
  id_rs_win = create_newwin((num_id_rs_regs+2),30,8,60,5);
  mvwprintw(id_rs_win,0,12,"ID/RS");
  wrefresh(id_rs_win);

  //instantiate a window to visualize RS stage
  rs_win = create_newwin((num_rs_regs+2),30,8,90,5);
  mvwprintw(rs_win,0,10,"RS STAGE");
  wrefresh(rs_win);

  //instantiate a window to visualize RS/EX signals
  rs_ex_win = create_newwin((num_rs_ex_regs+2),30,8,120,5);
  mvwprintw(rs_ex_win,0,12,"RS/EX");
  wrefresh(rs_ex_win);
...
void parsedata(int history_num_in){
  ...
  // Handle updating the ID/RS window
  for(i=0;i<num_id_rs_regs;i++){
    if (strcmp(id_rs_contents[history_num_in][i],
                id_rs_contents[old_history_num_in][i]))
      wattron(id_rs_win, A_REVERSE);
    else
      wattroff(id_rs_win, A_REVERSE);
    mvwaddstr(id_rs_win,i+1,strlen(id_rs_reg_names[i])+3,id_rs_contents[history_num_in][i]);
  }
  wrefresh(id_rs_win);
  ...
  // Handle updating the RS/EX window
  for(i=0;i<num_rs_ex_regs;i++){
    if (strcmp(rs_ex_contents[history_num_in][i],
                rs_ex_contents[old_history_num_in][i]))
      wattron(rs_ex_win, A_REVERSE);
    else
      wattroff(id_rs_win, A_REVERSE);
    mvwaddstr(id_rs_win,i+1,strlen(rs_ex_reg_names[i])+3,rs_ex_contents[history_num_in][i]);
  }
  wrefresh(rs_ex_win);
...
int processinput(){
  ...
  static int id_rs_reg_num = 0;
  static int rs_reg_num = 0;
  static int rs_ex_reg_num = 0;
  ...
  readbuffer[strlen(readbuffer)-1] = 0;
	...
	// t = timestamp
	// z = reset
	// a = ARF registers <- subject to change
  ...
  if(strncmp(readbuffer,"t",1) == 0){
    ...
    // We are getting an ID/RS register

    // If this is the first time we've seen the register,
    // add name and data to arrays
    if (!setup_registers) {
      parse_register(readbuffer, id_rs_reg_num, id_rs_contents, id_rs_reg_names);
      mvwaddstr(id_rs_win,id_rs_reg_num+1,1,id_rs_reg_names[id_rs_reg_num]);
      waddstr(id_rs_win, ": ");
      wrefresh(id_rs_win);
    } else {
      sscanf(readbuffer,"%*c%s %d:%s",name_buf,&tmp_len,val_buf);
      strcpy(id_rs_contents[history_num][id_rs_reg_num],val_buf);
    }
    id_rs_reg_num++;
    ...
    // We are getting an RS/EX register

    // If this is the first time we've seen the register,
    // add name and data to arrays
    if (!setup_registers) {
      parse_register(readbuffer, rs_ex_reg_num, rs_ex_contents, rs_ex_reg_names);
      mvwaddstr(rs_ex_win,rs_ex_reg_num+1,1,rs_ex_reg_names[rs_ex_reg_num]);
      waddstr(rs_ex_win, ": ");
      wrefresh(rs_ex_win);
    } else {
      sscanf(readbuffer,"%*c%s %d:%s",name_buf,&tmp_len,val_buf);
      strcpy(rs_ex_contents[history_num][rs_ex_reg_num],val_buf);
    }
...
//10 int parameters
void initcurses(int if_regs, int if_id_regs, int id_regs, int id_rs_regs, int rs_regs,
                int rs_ex_regs, int ex_regs, int ex_mem_regs, int mem_regs, int misc_regs){
  int nbytes;
  int ready_val;
  ...
  num_id_regs = id_regs;
  num_id_rs_regs = id_rs_regs;
  num_rs_regs = rs_regs;
  num_rs_ex_regs = rs_ex_regs;
  num_ex_regs = ex_regs;
  ...
  if(childpid == 0){
    ...
    id_rs_contents    = (char***) malloc(NUM_HISTORY*sizeof(char**));
    rs_contents       = (char***) malloc(NUM_HISTORY*sizeof(char**));
    rs_ex_contents    = (char***) malloc(NUM_HISTORY*sizeof(char**));
    ...
    id_rs_reg_names   = (char**) malloc(num_id_rs_regs*sizeof(char*));
    rs_reg_names      = (char**) malloc(num_rs_regs*sizeof(char*));
    rs_ex_reg_names   = (char**) malloc(num_rs_ex_regs*sizeof(char*));
    ...
    for(;i<NUM_HISTORY;i++){
      ...
      id_rs_contents[i]   = (char**) malloc(num_id_rs_regs*sizeof(char*));
      rs_contents[i]      = (char**) malloc(num_rs_regs*sizeof(char*));
      rs_ex_contents[i]   = (char**) malloc(num_rs_ex_regs*sizeof(char*));
